        ...
        <div class="action-board">
            ...
            <div class="btn-group">
                <div class="action-item" :class="{ 'is-active': isLiked }" @click="handtoggleLike">
                    <el-icon :size="20">
                        <Pointer v-show="!isLiked" />
                        <Pointer v-show="isLiked" class="filled" />
                    </el-icon>
...
<script setup lang="ts">
    ...
    const handtoggleLike = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                ElMessage.error(result.message || `${actionText}失败，请稍后重试`)
            }
        } catch (error) {
    ...
    const handLeft = () => {
        if (props.next?.id) {
            goArticleDetail(props.next.id)
        }
    }
    const handRigth = () => {
        if (props.prev?.id) {
            goArticleDetail(props.prev.id)
        }
    }
